"""
Author: HaoZhi
Date: 2022-10-20 14:17:59
LastEditors: HaoZhi
LastEditTime: 2022-10-20 14:46:47
Description: 
"""
...
class SSD300(nn.Module):
    ...
    def forward(self, image):
        ...
        conv8_2_feats, conv9_2_feats, conv10_2_feats, conv11_2_feats = self.aux_conv(
            conv7_feats
        )
    ...
    def detect_objects(
        ...
        for i in range(batch_size):
            ...
            image_labels = list()
            image_scores = list()
            max_scores, best_labels = predicted_scores[i].max(dim=1)
            ...
            if len(image_boxes) == 0:
                image_boxes.append(
                    torch.FloatTensor([[0.0, 0.0, 1.0, 1.0]]).to(self.device)
                )
                image_labels.append(torch.FloatTensor([0]).to(self.device))
                image_scores.append(torch.FloatTensor([0.0]).to(self.device))
        ...
        return all_images_boxes, all_image_labels, all_images_scores


class MultiBoxLoss(nn.Module):
    def __init__(
        self, priors_cxcy, threshold=0.5, neg_pos_ratio=3, alpha=1.0, device=None
    ) -> None:
        super().__init__()
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        for i in range(batch_size):
            ...
            overlap_for_each_prior, object_for_each_prior = overlap.max(
                dim=0
            )  # (n_priors) 每一个anchor被分配到了那个目标上，返回的两个值，前者代表与被分配目标的iou，后者代表被分配目标的id，主义真id的取值范围是[0, n_obj)
            _, prior_for_each_object = overlap.max(
                dim=1
            )  # (n_obj), 为了保证每一个box都会被分到对应的anchor，这里对overlap的dim = 1取最大，返回的第二项代表对应anchor的id
            object_for_each_prior[prior_for_each_object] = torch.LongTensor(
                range(n_objects)
            ).to(
                self.device
            )  # 如果有没有被分配到anchor的obj，这里重新对上两步的结果进行修改，因为obj的id是从0~n_obj，所以用range进行幅值
            overlap_for_each_prior[
                prior_for_each_object
            ] = 1.0  # 通过修改overlap的值，保证这些被分配的anchor的iou = 1.0， 一定大于阈值。
        ...
        n_hard_negative = self.neg_pos_ratio * n_positives
        ...
        hard_negatives = hardness_ranks < n_hard_negative.unsqueeze(1)
